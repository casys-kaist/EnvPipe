...
from deepspeed.runtime.constants import ENVPIPE_SCHEDULING_1F1B
...
class PipelineEngine(DeepSpeedEngine):
    ...
    def train_batch(self, data_iter=None):
        ...
        # if self.global_rank % self.device_cnt == 0:
        #     for device_id in range(self.device_cnt):
        #         handle = nvmlDeviceGetHandleByIndex(device_id)
        #         self.energy[device_id] = nvmlDeviceGetTotalEnergyConsumption(
        #             handle)
        ...
        #     for device_id in range(self.device_cnt):
        #         handle = nvmlDeviceGetHandleByIndex(device_id)
        #         current_energy = nvmlDeviceGetTotalEnergyConsumption(handle)
        #         self.energy[device_id] = current_energy - \
        #             self.energy[device_id]

        #     self.total_energy += sum(self.energy)

        self.timers('train_batch').stop()

        if self.global_steps % self.steps_per_print() == 0:
            if self.global_rank == 0:
                elapsed = self.timers('train_batch').elapsed(
                    reset=True) / 1000.0
                iter_time = elapsed / self.steps_per_print()
                tput = self.train_batch_size() / iter_time

                # Energy consumption per step
                # avg_energy = self.total_energy / self.steps_per_print()
                # self.total_energy = 0

                # Skip first step
                # if self.global_steps != self.steps_per_print():
                #     print(f"[RESULT] {tput:0.3f} ", int(avg_energy))
        ...
        #     sys.exit()
            ...
            #       f'avg energy: {int(avg_energy):0.1f}')
        ...
        # DEBUG
        if not self.energy_profiler.is_profiling:
            if not self.execution_grid.finish_reconfigure():
                if self.global_rank == 0:
                    print(self.global_steps)
                    print(self.execution_grid)
    ...
    def _recv_tensor_meta(self, send_stage):
        ...
        # List or tuple of tensors
            ...
            buffers = self._allocate_buffers(
                recv_shapes_and_dtypes, num_buffers=1)[0]
    ...
    def _exec_send_grads(self, buffer_id, async_op=False):
        if self.wall_clock_breakdown():
        ...
        if isinstance(inputs, torch.Tensor):
            ...
            p2p.send(inputs.grad, self.prev_stage, async_op=async_op)
        else:
            # XXX terrible hacky branch
            if self.is_grad_partitioned:
                # First two sends are partitioned gradient
                p2p.send(inputs[0], self.prev_stage, async_op=async_op)
                p2p.send(inputs[1], self.prev_stage, async_op=async_op)
            else:
                for idx, buffer in enumerate(inputs):
                    ...
                    p2p.send(buffer.grad, self.prev_stage, async_op=async_op)
