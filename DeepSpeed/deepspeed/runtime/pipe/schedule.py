...
class PipeSchedule(ABC):
    ...
    @property
    def is_second_last_stage(self):
        """True if the configured ``stage_id`` is the 2nd last stage in the pipeline."""
        return self.stage_id == self.stages - 2
            ...
            if is_forward:
                ...
                if self._valid_micro_batch(prev_micro_batch_id) and self._valid_stage(
                    ...
                    cmds.append(EventRecord(is_start=True,
                                            ...
                                            is_forward=False,
                                            is_send=True))
                    cmds.append(SendGrad(prev_buffer, async_op=False))
                    cmds.append(EventRecord(is_start=False,
                                            micro_batch_id=prev_micro_batch_id,
                                            is_forward=False,
                                            is_send=True))
                ...
                if not send_activation_queue.empty() and self._valid_stage(
                    ...
                    cmds.append(SendActivation(
                        self._buffer_idx(micro_batch_id_to_send), async_op=False))
                    cmds.append(EventRecord(is_start=False,
                                            micro_batch_id=micro_batch_id_to_send,
                                            is_forward=True,
                                            is_send=True))
