...
def split_half_float_double_sparse(tensors):
    ...
    for t in tensors:
        assert t.type(
        ) in supported_types, f"attempting to reduce an unsupported grad type: {t.type()}"
...
class DeepSpeedEngine(Module):
    ...
    def __init__(
        ...
        # False --> Use torch.dist, True --> Use ds.comm backend.
        ...
        self.param_names = {param: name for name,
        ...
        for p in self.module.parameters():
            ...
            if is_moe_param(p):
                if torch.is_tensor(p) and is_replicated(p):
                    dist.broadcast(p,
                                   groups._get_expert_broadcast_src_rank(
                                       p.group_name),
    ...
    def _configure_optimizer(self, client_optimizer, model_parameters):
        if client_optimizer is not None:
            if isinstance(client_optimizer, tuple(self._supported_optims())):
                ...
                log_dist(
                    'Using client callable to create basic optimizer', ranks=[0])
        else:
        ...
        if self.zero_optimization():
            ...
            assert not (self.fp16_enabled() or self.bfloat16_enabled(
    ...
    def _configure_fp16_optimizer(self, optimizer):
        ...
        if isinstance(optimizer, fused_opts) \
            ...
            if self.dynamic_loss_scale():
                log_dist(
                    "Creating fp16 optimizer with dynamic loss scale", ranks=[0])
    ...
    def _configure_zero_optimizer(self, optimizer):
        ...
        assert self.communication_data_type in (
            torch.float16, torch.bfloat16), "ZeRO supports only 'communication_data_type': ['fp16', 'bfp16']"
        timers = self.timers if self.wall_clock_breakdown() else None
        ...
        if zero_stage <= ZeroStageEnum.gradients:
            ...
            assert not isinstance(
                optimizer, DummyOptim), "zero stage 2 requires an optimizer"
    ...
    def forward(self, *inputs, **kwargs):
        ...
        if self.autotuning_profile_model_info():
            ...
            see_memory_usage("Engine before forward",
                             force=self.memory_breakdown())
        ...
        if self.autotuning_profile_model_info():
            ...
            see_memory_usage("Engine after forward",
                             force=self.memory_breakdown())
        return loss
    ...
    def backward(self,
        ...
        see_memory_usage("Engine before backward",
                         force=self.memory_breakdown())
        ...
        see_memory_usage("Engine after backward",
                         force=self.memory_breakdown())
    ...
    def step(self, lr_kwargs=None):
        ...
        if self.is_gradient_accumulation_boundary():
            ...
            if (self.eigenvalue_enabled() and
                (self.gas_boundary_ctr %
                 self.eigenvalue_gas_boundary_resolution() == 0)
        ...
        if self.wall_clock_breakdown() or self.flops_profiler_enabled():
            ...
            if self.is_gradient_accumulation_boundary():
                ...
                if self.has_moe_layers:
                    fwd_time = self.timers(
                        FORWARD_GLOBAL_TIMER).elapsed(reset=False)
                    ...
                    for local_expert_id in range(num_local_experts):
                        ...
                        for key in list(expert_state_dict.keys()):
                            local_key = key.replace(
                                ...
                                f'{moe_str_prefix}{local_expert_id}')
                            expert_state_dict[local_key] = expert_state_dict.pop(
                                key)
                        state_dict.update(expert_state_dict)
                    moe_layer_id += 1
            ...
            if isinstance(module, MoE):  # and deepspeed.comm.get_rank() == 0:
                ...
                for key in list(moe_state_dict.keys()):
                    ...
                    experts_state_dict[str(
                        global_expert_id)][expert_key] = truncated
    ...
    def _get_zero_param_shapes(self):
        ...
        for bit16_group in bit16_groups:
            ...
            for param in bit16_group:
                cnt += 1
                numel += param.ds_numel if hasattr(param,
                                                   "ds_numel") else param.numel()
                shape = param.ds_shape if hasattr(
                    param, "ds_shape") else param.shape
