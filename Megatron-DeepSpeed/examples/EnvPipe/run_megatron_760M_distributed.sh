...
if [ $# -ne 8 ]
  then
    ...
    ./run_model.sh <MICROBATCH_SIZE> <MINIBATCH_SIZE> <ENVPIPE_TYPE> <ENVPIPE_SCHEDULING> <ENVPIPE_RECONFIGURE> <ENVPIPE_GPU> <DP_SIZE> <PP_SIZE> \n"
    exit 1
fi
...
# Increase this value if performance degrades
RECONFIGURE_THRESHOLD_SCALE=10

# Multi-node setting
NNODES=2
NPROC_PER_NODE=4
NODE_RANK=0 # Change this value to local rank 
...
if [ "${USE_INTERNAL_DATA}" = "true" ]; then
    ...
    DATA_BLEND=/data/webtext_text_document
fi
###############################################################################
data_options=" \
...
distributed_options="--nproc_per_node ${NPROC_PER_NODE} \
		     --nnodes ${NNODES} \
		     --node_rank ${NODE_RANK} \
		     --master_addr ${MASTER_ADDR} \
		     --master_port ${MASTER_PORT}"

run_cmd="python -m torch.distributed.launch ${distributed_options} ${DIR}/../../pretrain_gpt.py ${megatron_options} ${data_options} ${deepspeed_options}"
echo ${run_cmd}
eval ${run_cmd}
set +x
